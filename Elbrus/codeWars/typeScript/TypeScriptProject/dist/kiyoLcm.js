"use strict";
function kiyoLcm(a) {
    const noEvenNumbers = a.map(el => el.filter(el => el % 2 !== 0 && typeof el !== 'string'))
        .map(el => {
        return el.reduce((acc, curr) => {
            return acc + curr;
        }, 0);
    });
    const nok = (noEvenNumbers) => {
...
const randomNumberkiyoLcm = kiyoLcmArr.map((el) => el.map((el) => el = Math.floor(Math.random() * (50 - 1) + 1)));
